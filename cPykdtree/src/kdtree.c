...
int main()
{
    ...
    free(x);
    printf("N: %d nboxes %d\n",tree->N, tree->nboxes);
    printf("estimated size: %d bytes\n", 36*tree->N + 80*tree->nboxes);

    printf("KDtree computed, x freed. Type something then enter.\n");
    scanf("%d",&a);
    
    write_tree(*tree,"temp.test.kd");
    free_tree(tree);
    printf("tree written, then freed. Type something then enter.\n");
    scanf("%d",&a);

    Tree *tree2 = ALLOC(1, Tree, "tree2");
    read_tree("temp.test.kd",tree2);
    printf("tree read. Type something then enter.\n");
    scanf("%d",&a);
    ...
    /* int n = locatenear(tree2, pt, 2.0, ind, dist, 1000 ); */
    /* for( i=0; i<n; i++ ){ */
    /* 	printf("%i  %f \n",ind[i],dist[i]); */
    /* } */
    ...
    /* FLOAT *dist=malloc(nb*sizeof(FLOAT)); */
